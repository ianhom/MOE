...
- MOE中，“初始化”也可以认为是一个事件，可以拥有对应事件处代码。但这里需要注意一点，如果采用switch来找对应的事件处理代码，不建议像习惯那样把初始化代码放在最前面，反而应该放在最后，这是考虑到switch分支的特性（根据case顺序依次判断），将执行最不频繁的分支放到最后，有利于运行时的效率。
...
- 为什么要出开发板，而且要完成度高的开发板?因为没有无缘无故的爱。一个os，平台还是生态，都是先给用户提供益处，才有共同的建设对于用户分两类，即用量产的用户，为他们提供的越全越好，协议栈，驱动，框架甚至资源统计分析数据都有最好；另一类用户是玩家用户，对内部感兴趣，人数虽少，但影响是非常大的。
...
- 重置OS不是目的，理解，演变，定向优化是不错的方向，过程有收获，形成服务亦有收益。
