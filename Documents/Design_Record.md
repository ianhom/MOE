## 关于MOE
- 简单来说，MOE就是“Minds Of Embedded”含义的操作系统。
- 简单来说，MOE就是一个多任务事件驱动型的操作系统。
- 简单来说，MOE就是一个目标useful & helpful & handy的操作系统。 
- 简单来说，MOE就是以我大女儿“萌萌（MOE）”命名的操作系统。:smile:
...
- 还有一个注意点就是，PT上下文不是通过栈来保存的，所以依靠栈的局部变量不能再PT中使用（在Yeild之前就用完的局部变量除外），不过可以通过static修饰为静态局部变量，即可正常使用。

## 关于事件响应风格应用程序
- 改风格的应用程序主体并不像以往的程序那样“面相过程”，程序并不描述一个时序或流程，而是将过程中对应的处理动作划归为对事件的相应。这样虽然结构零散，但思路反而更清晰。对于无真正多线程的机制下，这种风格的应用恰恰是多应用运行的真相，此问题后续讨论。
...
## 关于PT+事件响应风格
- 如上所述，PT风格更接近于入门时编写的C程序---一个自上而下的风格。而事件响应则没有行文顺序，来什么事件，执行响应的代码，更像是以往的中断。那对于“主程序+中断”的传统风格，我们现在也有了“PT+事件响应”。
...
- LEON将是一个通讯协议栈集合（TCP/IP, 6LoWPAN, LoRaWan, ZigBee, BLE...）随着LEON的成长，MOE也将成熟。
...
- 重复造轮子不完全是一件浪费精力的事，技能和思路需要锻炼，即使所锻炼的内容不会使用到。一个简单的例子：去健身房挥霍体力也不是为了更好做体力活，而是为了有个更健康的身体。重复造轮子也是了“健康”。
- 对我而言，我很庆幸自己有机会写了MOE，在编写的过程不断强化嵌入式思想，正如MOE的对外含义“Minds Of Embedded”，个人能力和自信都在默默增加，编写MOE所带来的益处远远超过了MOE本身的实用性。同时，自制调度/OS的小伙伴也越来越多，能有更多的人一起交流，是件很开心的事，学习他山之石，也是受益匪浅。
