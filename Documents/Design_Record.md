...
- 在RTOS中，任务一般有“运行态”、“就绪态”和“阻塞态”几个状态，在MOE中也有对应的表现形式，当MOE任务正在运行时，MOE任务处于“运行态”；当对应到某个MOE任务的事件处在事件队列时，该MOE任务处于“就绪态”；如果事件队列中没有事件指向某个MOE任务，那该MOE任务可以认为是“阻塞态”
 
- MOE中的任务件通讯机制有事件和消息，这点和rtos类似。关于这点，事件和消息模块会设计成兼容目前协作型内核和后续实现的抢占型内核
 
