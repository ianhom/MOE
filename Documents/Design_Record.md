...
- 暂且按这种粗扩的方式划分，考虑到接口size优化，后续在看是否将配置和初始化或配置和控制命令放到一起。
...
- 便捷的调试方式能大幅提高开发效率，即使在MOE的内核开发中，因为少有硬件资源操作，所以MOE模块的工作情况都是通过串口printf来实现的。printf调试方式简单实用，不过任然有三个问题：
      ...
      3. 不同文件有各自不同的调试等级要求。
- 对于不喜欢防御性代码那种只把问题抛出而不一定能等到及时处理的风格的同学，MOE也提供了assert方法

## 关于演化方向
- 增加RTOS实时内核，虽然MOE的方向并不是追求实时性，但是实时性依旧是我想学的个一个方向，可以为MOE提供可选的实时内核
- 增加脚本语言解释器（Python，JavaScript，Lua...）一直想给MOE增加交互，使用现有的脚本语言是个不错的选择 
