...
{  
    ...
    MOE_CHECK_RET_ST(!(u8TaskID > MAX_TASK_NUM),"Task ID should be valid");
...
static uint8 Moe_Event_Setting(uint8 u8TaskID, uint8 u8Evt, uint8 u8Urg)
{
    ...
    MOE_CHECK_RET_ST((MAX_QUEUE_EVT_NUM != sg_u16EvtCnt), "Event queue is full");
...
{  
    ...
    MOE_CHECK_RET_ST((NULL != ptEvt),"Event pointer should NOT be NULL");

    /* If there is no event */
    MOE_CHECK_RET_ST((0 != sg_u16EvtCnt), "Event count is 0");
...
static uint8 Moe_Event_Setting(uint8 u8TaskID, uint8 u8Evt, uint8 u8Urg)
{
    ...
    MOE_CHECK_RET_ST((sg_u16EvtCnt < sg_u16EvtCntMax),"Event queue is full");
...
static uint32 Moe_Event_Queue_Block_Add(void)
{
    ...
    MOE_CHECK_RET_ST((NULL != ptAdd),"Malloc failed")!!
